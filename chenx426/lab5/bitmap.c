...
struct pixel **read_pixel_array(FILE *image, int pixel_array_offset, int width, int height) {
  struct pixel **read_pixel_array = malloc(height*sizeof(struct pixel*));
  fseek(image,pixel_array_offset,SEEK_SET);
  for(int i=0;i<height;++i)
  {
    read_pixel_array[i]=malloc(width*sizeof(struct pixel));
    for(int j=0;j<width;++j)
      fread(&read_pixel_array[i][j], sizeof(struct pixel), 1, image);
  }
  return read_pixel_array;
}
